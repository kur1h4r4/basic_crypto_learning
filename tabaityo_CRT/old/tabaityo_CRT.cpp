...
int main(){
    ...
    // dp, dq算出
    // divTの剰余だけほしいので商は一時的な変数に格納
    BigNum<BASE_N> q_dummy_p, q_dummy_q;
    divT<BASE_N, BASE_N>(q_dummy_p.digits.data(), dp.digits.data(), d.digits.data(), pminus.digits.data());
    divT<BASE_N, BASE_N>(q_dummy_q.digits.data(), dq.digits.data(), d.digits.data(), qminus.digits.data());
    
    // q1算出
    q1 = modinv(q, p);

    BigNum<BASE_N> mp = modexp(c, dp, p);
    BigNum<BASE_N> mq = modexp(c, dq, q);

    // h = (mp - mq) * qinv mod p
    BigNum<BASE_N> h_diff;
    // mp - mqが負になる場合はmodpを足して調整。
    ...
    uint64_t widebuf_h[BASE_N * 2] = {};
    mulT<BASE_N>(widebuf_h, h_diff.digits.data(), q1.digits.data());

    BigNum<BASE_N> h, q_dummy_h;
    divT<BASE_N * 2, BASE_N>(q_dummy_h.digits.data(), h.digits.data(), widebuf_h, p.digits.data());

    // 平文mの算出
    // m = mq + h * q
    uint64_t widebuf_final[BASE_N * 2] = {};
    mulT<16>(widebuf_final, h.digits.data(), q.digits.data());

    BigNum<BASE_N> final_m;
    std::memcpy(final_m.digits.data(), widebuf_final, sizeof(uint64_t) * BASE_N);
